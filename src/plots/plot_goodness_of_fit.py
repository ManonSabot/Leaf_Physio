...
def main(identity):
    ...
    # figure
    fname = os.path.join(opath, 'goodness_of_fit.jpg')
...
def studentize(df, vars=['gs', 'Ci', 'E', 'A', 'Pleaf']):
    ...
    icol = df.columns.get_loc('A')
    df.iloc[:, icol] = df.iloc[:, icol].subtract(mean['A']).div(std['A'])

    for e in columns:

        icol = df.columns.get_loc(e)
        df.iloc[:, icol] = df.iloc[:, icol].subtract(mean[e]).div(std[e])
...
if __name__ == "__main__":
    ...
    # user input
    site_spp = 'Richmond_Eucalyptus_saligna'
