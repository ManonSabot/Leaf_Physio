...
def main(site_spp=site_spp_order(), subsel1=None, subsel2=None):
    ...
    # data to plot
    df = pd.read_csv(os.path.join(os.path.join(os.path.join(base_dir,
                     'output'), 'all_site_spp'),
    ...
    figname = os.path.join(opath, 'A_E_subsel.jpg')

    if not os.path.isfile(figname):
        A_E_relationships(df, figname, subsel1)

    # Ci(gs) contours
    figname = os.path.join(opath, 'Ci_gs.jpg')
    ...
    figname = os.path.join(opath, 'gs_LWP.jpg')

    if not os.path.isfile(figname):
        gs_LWP_functional(df, figname, site_spp)

    # iWUE summary example plots
    figname = os.path.join(opath, 'iWUE_subsel.jpg')

    if not os.path.isfile(figname):
        iWUE_relationships(df, figname, subsel2)

    # VPD closure plots
    figname = os.path.join(opath, 'VPD_closure.jpg')

    if not os.path.isfile(figname):
        VPD_closure(df.copy(), figname, site_spp)

    # LWP errors
    figname = os.path.join(opath, 'LWP_errors.jpg')
...
def gs_LWP_functional(df, figname, keep):
    ...
    for i, what in enumerate(keep):
        ...
        ipath = os.path.join(os.path.join(get_main_dir(), 'input'),
                             'all_site_spp')
        ref, __ = read_csv(os.path.join(ipath, '%s.csv' % (what)))
...
def VPD_closure(df, figname, keep):
    ...
    ax.hlines(0.5, 2., all_max['VPD'].max(), linestyle='--', linewidth=2.,
              zorder=20)
    ax.vlines(2., 0.1, 0.5 + 0.0065, linestyle='--', linewidth=2., zorder=20)
    ax.vlines(all_max['VPD'].max(), 0.1, 0.5 + 0.0065, linestyle='--',
    ...
    ax.plot(all_plot['VPD'], all_plot['gs'], lw=5., color='#c0c0c0',
            alpha=0.6)
    ...
    # tighten
    ax.set_xlim(all['VPD'].min(), all['VPD'].max())
    ax.set_ylim(0.1, np.amax(np.minimum(all['gs'] + all['gs'].std(), 1.)) - 0.1)
    ...
    for i, mod in enumerate(models):
        ...
        high['gs(%s)' % (mod)].plot.kde(ax=ax, color=colours[i], alpha=0.9)

        # grabbing x and y data from the kde plot
        kde = gaussian_kde(high['gs(%s)' % (mod)].dropna())
        x = np.linspace(high['gs(%s)' % (mod)].min(),
                        high['gs(%s)' % (mod)].max(), 100)
        y = kde.evaluate(x)
        ...
        ax.vlines(0, 0, 1.0025 * y[np.argmax(y)], lw=0.75, color='#5a6576',
                  zorder=20)
...
def LWP_error_plots(df, figname):
    ...
    for c in select:
        ...
        df.loc[df[c] >= 0., c] = np.nan  # mask nonsense
        df.loc[df[c] < -999., c] = np.nan  # mask nonsense

    range = (df.groupby('site_spp')['Pleaf'].min() -
             df.groupby('site_spp')['Pleaf'].max()).abs().max()
    ax.fill_between(np.arange(-0.5, len(model_order()) * 2.), -0.9 * range,
                    0.9 * range, facecolor='#c0c0c0', edgecolor='none',
                    alpha=0.2)
    ax.fill_between(np.arange(-0.5, len(model_order()) * 2.), -0.1 * range,
    ...
    for i, mod in enumerate(model_order()):
        ...
        try:
            ...
            for site_spp in df['site_spp'].unique():

                valid = np.logical_and(df['site_spp'] == site_spp,
                                       df['Pleaf(%s)' % (mod)] <
                                       1.01 * df['Ps'])
                LWP = (((df['Pleaf(%s)' % (mod)] - df['Pleaf'])[valid])
                       .dropna().to_list())
                bp = ax.boxplot(LWP, positions=[pos],
                                widths=0.9 / len(df['site_spp'].unique()),
                ...
                pos += 0.95 / len(df['site_spp'].unique())
    ...
    # ax labels
    render_ylabels(ax, r'$\Delta$$\Psi_l$', 'MPa')

    # set the subplot's title
    txt = ax.annotate(r'\textit{Eucalyptus}', xy=(0.99, 0.985),
    ...
    handles = [Patch(facecolor=c, edgecolor='none')
               for c in plt.rcParams['axes.prop_cycle'].by_key()['color']]
    labels = [which_model(mod) for mod in model_order()]
    ax.legend(handles, labels, bbox_to_anchor=(1.05, 0.9), loc=2,
...
if __name__ == "__main__":
    ...
    # user input
    subsel1 = ['Richmond_Eucalyptus_dunnii', 'Richmond_Eucalyptus_saligna',
               'Richmond_Eucalyptus_cladocalyx',
               'Corrigin_Eucalyptus_capillosa']
    subsel2 = ['Richmond_Eucalyptus_dunnii', 'Richmond_Eucalyptus_saligna']

    main(subsel1=subsel1, subsel2=subsel2)
