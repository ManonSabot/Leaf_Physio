...
def main(to_plot, what):
    ...
    dirname = os.path.join(os.path.join(base_dir, 'output'), 'all_site_spp')
    ...
    if what == 'All':  # plot the overview of skill
        fname = os.path.join(opath, 'summary_of_skill.jpg')
...
def overview_of_skill(df, figname):
    ...
    for i, metric in enumerate(df['metric'].unique()):
        ...
        colours = np.asarray(['w', ] * len(model_order()) * 3, dtype='object')
