...
"""
Code that runs the met-driven model simulations where the models have
been calibrated to observations of gs, and also computes metrics of
...
__title__ = "Met-driven model simulations"
...
def main(runsim=False, models=['Medlyn', 'ProfitMax', 'TpDep_d','TpDep_u', 'Medlyn_wGmin', 'Tair_reg', 'ConstGsL',
                               'ConstGsM','ConstGsH']):
    ...
    all stored under 'output/all_site_spp/'.
    ...
    # path to input files
    ipath = os.path.join(os.path.join(base_dir, 'input'), 'HW_drought_test')
...
def combine_dfs(df1, df2, df3, identity):
    ...
    df2 = df2[[c for c in list(df2) if ((len(df2[c].unique()) > 1) or
              ('Rublim' in c) or ('gb' in c) or ('Pleaf' in c) or ('gs' in c) or ('hod' in c) or ('VPD' in c)
                                        or ('Ps' in c))]]
...
def compute_performance_metrics(df, opath):
    ...
    if not os.path.isfile(fname):
        bics = performance(df, which='RBIC')
...
if __name__ == "__main__":
    ...
    models = ['Medlyn', 'ProfitMax', 'TpDep_d', 'TpDep_u', 'Medlyn_wGmin', 'Tair_reg', 'ConstGsL', 'ConstGsM',
              'ConstGsH']
