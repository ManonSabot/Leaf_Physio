...
energy balance conditions are satisfied. Incorporates temperature dependence of gmin.
...
def Tdep_peak(p, sw, photo='Farquhar', res='low', iter_max=40,
    ...
    if Tleaf < p.Tp:    # solve for gmin (Cochard 2021)
    ...
    while True:
        ...
        # alternatively may calculate gmax if stomatal traits are available - ask if being measured in Anne's experiment
        gs = np.maximum(cst.zero, gmin + (p.gmax - gmin)/(pow(Tleaf-p.Topt, 2) + 1))
    ...
    return An, Ci, rublim, trans, gs, gb, gmin, new_Tleaf, Pleaf
